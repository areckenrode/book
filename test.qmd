...
files <- files[order(as.numeric(gsub("\\D", "", basename(files))), decreasing = TRUE)]
...
x <- ggplot(df_books, aes(x = date, xend = date, y = 0, yend = 1)) +
  ...
  geom_point(y = 1, color = "#DD7596", size = 4) +  # Dots for books
...
df_books2 <- df_books |> 
  ...
  pivot_longer(cols = c("plot_pages", "plot_pages_neg"),
               ...
               values_to = "value") |> 
  arrange(name) |> 
  mutate(value = case_when( # add silly exception for two shortest books
    name == "The Cat Who Played Brahms" & type == "plot_pages" ~ 97,
    TRUE ~ value)) |> 
  mutate(label = case_when(
    value == max(value) & type == "plot_pages" ~ paste0("Longest book - ", name, "\n", pages, " pages"),
    value == min(value[value>0]) & type == "plot_pages" ~ paste0("Shortest book - ", name, "\n", pages, " pages"),
    TRUE ~ ""))
...
fake_dat <- goodreads_library_export |> 
  mutate(Binding = case_when(
    ...
    Binding == "Mass Market Paperback" ~ "Kindle Edition", # I happen to know those 2 were kindle
    TRUE ~ Binding)) |> 
  group_by(Binding) |> 
  count() |> 
  ungroup() |> 
...
w <- df_books3.1 |> 
 ...
 geom_text(aes(label = author, y = 0.1),  # Place text at Y = 0
            angle = 0, color = "#710A31", size = 4, hjust = 0) +
  scale_fill_manual(values = c("TRUE" = "#EEF0FC", "FALSE" = "#FBEFF3")) +
  theme_minimal() +
  theme(
    text = element_text(color = "#710A31"),
    axis.ticks.y = element_blank(),  # Remove axis ticks
    axis.ticks.x = element_line(color = "#710A31"),  # Keep Y-axis tick marks
    axis.text.y = element_blank(), # Remove y-axis text
  ...
  labs(x=NULL,y=NULL, title = "Repeat authors by number of books")
...
c <- ggplot(soup, aes(total_pages, factor(year))) +
  ...
  theme_void() +
  theme(legend.position = "none", 
        axis.text.y = element_text(color = "#710A31", size = 40), 
        text = element_text(size = 38)) +
  geom_vline(xintercept = 0, color = "#710A31", linewidth = 1)
